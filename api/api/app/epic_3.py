...
async def main(input_data: dict) -> dict:
    ...
    contacts: pd.DataFrame = await get_contact_data(
        input_data["contact_ids"], connection
    )
    campaigns: pd.DataFrame = await get_campaign_data(connection)

    df, keys = await create_df(contacts, campaigns)

    df_predictions = await predict(df, keys, model)
    # Convert the df to a dict
    df_predictions = df_predictions.to_dict("records")
    return df_predictions


async def get_contact_data(contact_ids: list, connection) -> pd.DataFrame:
    query_contacts = f"""
    SELECT 
    /*Contact*/
...
async def get_campaign_data(connection) -> pd.DataFrame:
    query_campaigns = """
    SELECT
...
async def create_df(contacts, campaigns) -> tuple:
...
async def predict(df, keys, model, min_prob=0.5, top_n=10) -> pd.DataFrame:
    predictions = model.predict(df)
    keys["prediction"] = predictions
    # Filter out the predictions that are lower than min_prob
    keys = keys[keys["prediction"] >= min_prob]
    # Sort the values
    keys.sort_values(by=["crm_contact_id", "prediction"], inplace=True, ascending=False)

    # Select the top n grouped
    keys = (
    ...
    keys["prediction"] = keys["prediction"].apply(lambda x: round(x, 4))
    keys.rename(
        columns={
