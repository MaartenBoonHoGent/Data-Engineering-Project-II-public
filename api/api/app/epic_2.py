...
async def check_table_name(table_name: str, conn) -> bool:
    """
...
async def main(file: UploadFile, table_name: str):
    """
    Main function
    """
    ...
    # Check in the database if the tablename exists

    connection = create_conn()
    table_exists = await check_table_name(table_name, connection)
    if not table_exists:
    ...
    path = os.path.normpath(os.path.join(DATAPATH, file_name))
    ...
    try:
        ...
        # Run the epic_2 function
        await epic_2(table_name, file_name)
...
def get_new_data(file_name: str, table_info: Dict) -> pd.DataFrame:
    ...
    path = os.path.normcase(os.path.join(DATAPATH, file_name))
